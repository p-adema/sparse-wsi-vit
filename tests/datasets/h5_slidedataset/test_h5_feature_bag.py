...
def mock_fast_extraction_data(tmp_path):
    ...
    with h5py.File(slide1_h5, "w") as f:
        ...
        f.create_dataset(
            "coords", data=np.random.randint(0, 10000, size=(10, 2)).astype(np.int32)
        )
    ...
    with h5py.File(slide2_h5, "w") as f:
        ...
        f.create_dataset(
            "coords", data=np.random.randint(0, 10000, size=(5, 2)).astype(np.int32)
        )
...
def test_h5_feature_bag_dataset_initialization(mock_fast_extraction_data):
    ...
    dataset = H5FeatureBagDataset(
        csv_path=csv_path, features_dir=features_dir, label_col_name="label"
    )
