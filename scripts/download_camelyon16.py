...
def download_camelyon16(output_dir: Path):
    ...
    logging.info(
        "This will take a VERY long time (approx 400 GBs). Progress will be printed below."
    )
    ...
    try:
        ...
        logging.error(
            "The 'wget' command was not found on your system. Please install it (e.g. `sudo apt install wget`)."
        )
        sys.exit(1)
    except subprocess.CalledProcessError as e:
        logging.error(f"Download failed with error: {e}")
        logging.info(
            "You can resume the download anytime by running this script again. 'wget' will automatically resume partial files."
        )
